...
class Backtester {
  ...
  async getRealHistoricalData(symbol, startDate, endDate) {
    try {
      logger.info(
        `📊 Fetching real historical data for ${symbol} from ${startDate} to ${endDate}`
      );
      ...
      const data = await this.historicalClient.getHistoricalData(
        symbol,
        period,
        barSize
      );
      ...
      logger.error(
        `❌ Error fetching real historical data for ${symbol}:`,
        error.message
      );
      throw error;
    }
  ...
  /**
   * Generate synthetic historical data for backtesting (fallback method)
  ...
  enterPosition(symbol, analysis, candle, results) {
    ...
    logger.debug(
      `📈 Entered ${
        analysis.signal
      } position: ${symbol} @ $${entryPrice.toFixed(2)}`
    );
  }
  ...
  exitPosition(symbol, candle, reason, results) {
    ...
    logger.debug(
      `📉 Exited position: ${symbol} @ $${exitPrice.toFixed(
        2
   ...
   */
  async runBacktest() {
    logger.info("🚀 Starting momentum strategy backtest with REAL data...");
    ...
    try {
      ...
      for (const symbol of testSymbols) {
        try {
          ...
          let historicalData;
          try {
            historicalData = await this.getRealHistoricalData(
              symbol,
              startDate,
              endDate
            );
            ...
            logger.warn(
              `⚠️ Failed to get real data for ${symbol}, using synthetic data: ${dataError.message}`
            );
  ...
  calculateOverallPerformance(symbolResults) {
    ...
    const sharpeRatio =
      returnStdDev > 0 ? (avgReturn - 0.02) / returnStdDev : 0;
  ...
  displayResults(overall, symbolResults) {
    logger.info("📊 BACKTEST RESULTS (REAL DATA)");
    ...
    logger.info(
      `Total Return: $${overall.totalReturn.toFixed(
        2
      )} (${overall.totalReturnPercent.toFixed(2)}%)`
    );
    logger.info(`Total Trades: ${overall.totalTrades}`);
    logger.info(`Win Rate: ${overall.winRate.toFixed(1)}%`);
    logger.info(
      `Average Return per Trade: $${overall.averageReturn.toFixed(2)}`
    );
    ...
    symbolResults.forEach((result) => {
      if (result.totalTrades > 0) {
        ...
        logger.info(
          `${result.symbol}: ${result.totalTrades} trades, ${winRate.toFixed(
            1
          )}% win rate, $${result.totalReturn.toFixed(2)} return`
        );
      }
    });
    ...
    if (allTrades.length > 0) {
      ...
      logger.info("\n🏆 Best Trade:");
      logger.info(
        `${bestTrade.symbol}: ${bestTrade.side} $${bestTrade.pnl.toFixed(
          2
      ...
      logger.info("\n💸 Worst Trade:");
      logger.info(
        `${worstTrade.symbol}: ${worstTrade.side} $${worstTrade.pnl.toFixed(
          2
        )} (${worstTrade.pnlPercent.toFixed(2)}%)`
      );
    }
...
async function main() {
    ...
    try {
        logger.info("🎯 Starting Real Data Backtesting System...");
        ...
        logger.info(
          "✅ Backtest completed successfully with REAL market data!"
        );

        logger.info("\n💡 Next Steps:");
        logger.info("1. Review the detailed results in the logs/ directory");
        logger.info("2. Analyze the performance metrics and trade details");
        logger.info(
          "3. Adjust strategy parameters in config/trading-config.json if needed"
        );
        logger.info("4. Consider running backtests on different time periods");
