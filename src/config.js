...
export class Config {
  ...
  constructor(options = {}) {
    ...
    this.rateLimit = Math.min(
      options.rateLimit || Config.DEFAULT_RATE_LIMIT,
      Config.MAX_RATE_LIMIT
    );
    this.verifySsl =
      options.verifySsl !== undefined ? options.verifySsl : false;
   ...
   */
  toString() {
    return `Config(host='${this.host}', port=${this.port}, protocol=${
      this.useHttps ? "https" : "http"
    }, rateLimit=${this.rateLimit}, verifySsl=${this.verifySsl})`;
  }
}
