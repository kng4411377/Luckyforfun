...
export class MomentumStrategy {
    constructor(config) {
        ...
        this.signals = new Map(); // Store recent signals
    }
    ...
    updatePriceHistory(symbol, price, volume) {
        ...
        const maxHistory = Math.max(
            this.config.momentum.lookbackDays * 24, // Assuming hourly data
            this.config.technicalIndicators.rsi.period + 10
        );
    ...
    analyze(symbol, marketData) {
        ...
        const momentum = TechnicalIndicators.momentum(prices, this.config.momentum.lookbackDays);
        const rsi = TechnicalIndicators.rsi(prices, this.config.technicalIndicators.rsi.period);
        const smaShort = TechnicalIndicators.sma(prices, this.config.technicalIndicators.movingAverages.short);
        const smaLong = TechnicalIndicators.sma(prices, this.config.technicalIndicators.movingAverages.long);
        const bollinger = TechnicalIndicators.bollingerBands(prices, this.config.technicalIndicators.bollinger.period);
    ...
    generateSignals(data) {
        ...
        // RSI signals
        if (rsi < this.config.technicalIndicators.rsi.oversold && signal !== 'SELL') {
            if (signal === 'HOLD') signal = 'BUY';
            strength += 20;
            reasons.push(`RSI oversold: ${rsi.toFixed(2)}`);
        } else if (rsi > this.config.technicalIndicators.rsi.overbought && signal !== 'BUY') {
        ...
        if (volumeRatio > 1.5) {
            strength += 10;
            reasons.push(`High volume confirmation: ${volumeRatio.toFixed(2)}x average`);
        ...
        // Price range check
        if (currentPrice < this.config.momentum.priceRange.min || 
    ...
    calculateExitLevels(signal, entryPrice) {
        ...
        const takeProfitPercent = this.config.riskManagement.takeProfitPercentage / 100;
    ...
    checkExitSignal(symbol, position, marketData) {
        ...
        const trailingPercent = this.config.riskManagement.trailingStopPercentage / 100;
        if (side === 'BUY') {
            const trailingStop = currentPrice * (1 - trailingPercent);
            if (trailingStop > position.stopLoss) {
                position.stopLoss = trailingStop;
                TradingLogger.logPosition('Updated trailing stop', symbol, { newStopLoss: trailingStop });
            }
        }
    ...
    addPosition(symbol, position) {
        ...
        TradingLogger.logPosition('Added position', symbol, position);
    }
    ...
    removePosition(symbol) {
        ...
        if (position) {
            ...
            TradingLogger.logPosition('Removed position', symbol, position);
        }
    }
}
